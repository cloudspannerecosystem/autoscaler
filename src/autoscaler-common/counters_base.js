  ...
  try {
    ...
    if (process.env.OTLP_COLLECTOR_URL) {
      switch (process.env.OLTP_IS_LONG_RUNNING_PROCESS) {
        ...
        default:
          throw new Error(
            `Invalid value for env var OLTP_IS_LONG_RUNNING_PROCESS: "${process.env.OLTP_IS_LONG_RUNNING_PROCESS}"`,
          );
      }
      logger.info(
        `Counters mode: ${exporterMode} OLTP collector: ${process.env.OTLP_COLLECTOR_URL}`,
      );
