...
exports.calculateNumNodes = (spanner) => {
  ...
  return baseModule.loopThroughSpannerMetrics(spanner, (spanner, metric) => {
    if (metric.value == metric.threshold) return spanner.currentNodes;  // No change

    var suggestedStep = (metric.value > metric.threshold ? spanner.stepSize : -spanner.stepSize); 
    if (metric.name === baseModule.OVERLOAD_METRIC && spanner.isOverloaded) suggestedStep = spanner.overloadStepSize;
